...
void ParsePath(const CHAR* spec,
  ...
  for (int i = path.begin; i < path_end; i++) {
    switch (spec[i]) {
      case '?':
        // Only match the query std::string if it precedes the reference fragment
...
// Converts a port number in a std::string to an integer. We'd like to just call
...
int DoParsePort(const CHAR* spec, const Component& component) {
  ...
  // Null-terminate the std::string and convert to integer. Since we guarantee
